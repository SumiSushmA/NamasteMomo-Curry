@php
$restaurant = $site['restaurant_name'] ?? 'Namaste MoMo & curry house';
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
...
<body style="font-family:Arial,sans-serif;line-height:1.6;color:#222;max-width:560px;margin:0 auto;padding:24px">
    ...
    <p style="margin:0 0 16px">
        ...
        @if($occasion)
            for <strong>{{ $occasion->name }}</strong>.
        @elseif($card->occasion === 'custom' && $card->occasion_label)
            for <strong>{{ $card->occasion_label }}</strong>.
        @else
            ({{ $design->name }} design).
        @endif
    </p>
