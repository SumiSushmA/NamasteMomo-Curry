...
<div class="adm-card" style="padding:22px;margin-bottom:18px;">
    <h3 style="font-size:17px;font-weight:600;margin-bottom:14px;">Occasions</h3>
    <p style="color:var(--muted);font-size:13px;margin-bottom:14px;">Each occasion has its own card colors and headline on the customer gift cards page.</p>

    <div style="display:grid;gap:8px;">
        @foreach($occasions as $occasion)
            @php($palette = $occasion->palette())
            <form action="{{ route('admin.gift-cards.occasions.update', $occasion) }}" method="POST" style="display:grid;grid-template-columns:48px 1fr 1.2fr 56px .75fr .75fr .75fr .75fr auto;gap:10px;align-items:center;">
                @csrf @method('PATCH')
                <div style="width:48px;height:38px;border-radius:8px;border:1px solid var(--line);background:linear-gradient(125deg, {{ $palette['start'] }} 0%, {{ $palette['mid'] }} 48%, {{ $palette['end'] }} 100%);"></div>
                <input name="name" value="{{ $occasion->name }}" required style="background:var(--ink-800);border:1px solid var(--line);border-radius:10px;padding:9px 10px;color:var(--cream);font-family:var(--sans);">
                <input name="headline" value="{{ $occasion->headline }}" required style="background:var(--ink-800);border:1px solid var(--line);border-radius:10px;padding:9px 10px;color:var(--cream);font-family:var(--sans);">
                <input name="emoji" value="{{ $occasion->emoji }}" placeholder="🎂" style="background:var(--ink-800);border:1px solid var(--line);border-radius:10px;padding:9px 10px;color:var(--cream);font-family:var(--sans);text-align:center;">
                <input name="bg_start" type="color" value="{{ $palette['start'] }}" style="width:100%;height:36px;background:var(--ink-800);border:1px solid var(--line);border-radius:10px;padding:3px;">
                <input name="bg_mid" type="color" value="{{ $palette['mid'] }}" style="width:100%;height:36px;background:var(--ink-800);border:1px solid var(--line);border-radius:10px;padding:3px;">
                <input name="bg_end" type="color" value="{{ $palette['end'] }}" style="width:100%;height:36px;background:var(--ink-800);border:1px solid var(--line);border-radius:10px;padding:3px;">
                <input name="text_color" type="color" value="{{ $palette['text'] }}" style="width:100%;height:36px;background:var(--ink-800);border:1px solid var(--line);border-radius:10px;padding:3px;">
                <label style="display:flex;align-items:center;gap:6px;font-size:12px;color:var(--sand);white-space:nowrap;"><input type="checkbox" name="is_active" value="1" @checked($occasion->is_active)> Active</label>
                <button type="submit" class="btn btn-ghost btn-sm">Save</button>
            </form>
        @endforeach
    </div>
...
<div class="adm-card" style="padding:8px;">
    <div class="adm-table-wrap">
        <table class="adm-table">
            <thead>
                <tr>
                    <th>Code</th><th>Occasion</th><th>Design</th><th>Recipient</th><th>Channel</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span style="font-size:13.5px;color:var(--sand);">{{ $g['occasion'] ?: '—' }}</span></td>
