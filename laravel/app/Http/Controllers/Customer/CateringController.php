...
class CateringController extends Controller
{
    ...
    {
        if ($url = ToastConfiguration::tablesUrl()) {
        ...
        return view('customer.catering.create', [
            ...
            'heroImage' => SiteContent::image('Catering hero image', 'catering spread'),
        ]);
    }
    ...
    public function orderPerPerson(Request $request): RedirectResponse
    {
        ...
        foreach ($groups as $groupId) {
            $picked = array_values(array_filter($validated['selections'][$groupId] ?? [], function ($name) use ($groupId) {
                return CateringMenu::isValidOption($groupId, $name);
            }));
            if ($picked) {
                $hasSelection = true;
            }
