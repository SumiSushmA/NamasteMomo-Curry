# Namaste MoMo & curry house

Full-stack website for **Namaste MoMo & curry house**, a Seattle-area restaurant serving Indian and Nepali cuisine. The app includes a customer-facing site (menu, ordering, reservations, gift cards, and more) and an admin dashboard to manage content, orders, and integrations.
...
### Admin dashboard (`/nmch-kitchen-portal` by default)

Set `ADMIN_PATH` in `.env` to a private path (not `/admin`). Default: `nmch-kitchen-portal`.
...
# 3. Database
touch database/namaste_momo_curry_house.sqlite   # if using SQLite (default in .env.example)
php artisan migrate
...
Visit `http://127.0.0.1:8000`. Admin login: `/nmch-kitchen-portal/login` (`indainnepaliadmin@kitchen.com` / `password` — change `ADMIN_PATH` and password in production).
...
| Variable | Purpose |
|----------|---------|
| `APP_NAME` | Site name (e.g. `Namaste MoMo & curry house`) |
...
MIT (Laravel framework). Application code for Namaste MoMo & curry house.
