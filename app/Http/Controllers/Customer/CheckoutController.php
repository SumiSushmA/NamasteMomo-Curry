...
class CheckoutController extends Controller
{
    ...
    private function formatCateringNotes(array $cartItems): string
    {
        ...
        $priceMap = \App\Data\CateringMenu::optionPriceMap();
        ...
        foreach (($perPerson['selections'] ?? []) as $groupId => $items) {
            ...
            $formatted = collect($items)->map(function ($name) use ($groupId, $priceMap) {
                $price = $priceMap[$groupId][$name] ?? 0;
                if ($price > 0) {
        ...
        $unit = \App\Data\CateringMenu::perPersonUnitPrice($perPerson['selections'] ?? []);
        $lines[] = 'Per-guest rate: $'.number_format($unit, 2).' (base + selections)';
